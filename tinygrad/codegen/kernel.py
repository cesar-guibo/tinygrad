...
class Opt:
  ...
  def real_axis(self, k:Kernel):
    ...
    if self.op is OptOps.UNROLL: return k.first_scan+self.axis
    if self.op in {OptOps.GROUP, OptOps.GROUPTOP}: return k.first_scan+k.group_for_scans+self.axis
    return self.axis
...
class Kernel:
  def __init__(self, ast:UOp, opts:Optional[Renderer]=None):
    ...
    self.scanops = dedup([x for x in ordered_parents(self.ast) if x.op is Ops.SCAN_AXIS])

    self.vars: List[Variable] = self.ast.variables()
    self.bufs: List[UOp] = [x for x in self.ast.parents if x.op in GroupOp.Buffer]

    # get earlybufs, before any scanops
    earlybufs: List[UOp] = [x for scanop in self.scanops for x in scanop.parents if x.op in GroupOp.Buffer]
    self.full_buf_index: int = self.bufs.index(earlybufs[0]) if earlybufs else 0
    # NOTE: full_shape can be wrong if there's a tree of scan
    ...
    self.scan_axis = {x: x.arg[1] for x in self.scanops}

    # move all scan axes to the end
    scan = set(functools.reduce(lambda acc,x: acc + x.arg[1], self.scanops, ()))
    permute = tuple(i for i in range(len(self.output_shape)) if i not in scan) + tuple(scan)
    self.scan_axis = {k: tuple(permute.index(x) for x in v) for k,v in self.scan_axis.items()}
    self.reshape_and_permute(None, permute)

    # parameters for optimization
    self.applied_opts: List[Opt] = []
    self.group_for_scans: int = 0
    self.upcasted: int = 0
  ...
  def copy(self):
    ...
    ret.applied_opts, ret.group_for_scans, ret.upcasted, ret.local_dims, ret.dont_use_locals = \
      self.applied_opts[:], self.group_for_scans, self.upcasted, self.local_dims, self.dont_use_locals
  ...
  def first_scan(self) -> int:
    return min(res) if (res:=functools.reduce(lambda a,x: a+[i for i in x if i < self.first_upcast], self.scan_axis.values(), [])) else self.first_upcast

  @property
  def first_upcast(self) -> int: return self.shape_len-self.upcasted

  @property
  def scanop(self) -> Optional[UOp]: return self.scanops[0] if len(self.scanops) > 0 else None
  ...
  def upcast_in_mid_reduce_axes(self) -> List[int]:
    return [j for j in range(self.first_scan, self.first_scan+self.group_for_scans) if self.full_shape[j] == self.sts[0].shape[j]]

  @property
  def global_dims(self) -> int: return self.first_scan-self.local_dims
  ...
  def colors(self) -> List[str]:
    ...
    # after global are local_dims; warp ones used in tensor cores must be closest to first_scan (cyan)
    colors += ["cyan"] * self.local_dims
    # between first_scan and first_scan + group_for_scans, they are either upcast mid reduce (white), or late upcasted (green)
    colors += ["white" if i in self.upcast_in_mid_reduce_axes else "green" for i in range(self.first_scan, self.first_scan + self.group_for_scans)]  # noqa: E501
    # between first_scan + group_for_scans and upcasted, they are reduce (red)
    colors += ["red"] * (self.first_upcast - (self.first_scan + self.group_for_scans))
  ...
  def shift_to(self, axis, amount, top=False, insert_before=None):
    ...
    permute = [i for i in range(insert_before) if i != move_axis] + [move_axis] + [i for i in range(insert_before, self.shape_len+1) if i != move_axis]
    self.scan_axis = {k: functools.reduce(operator.add, ((x,) if x < axis else ((x + 1,) if x > axis else (x, x + 1)) for x in v), tuple()) for k,v in self.scan_axis.items()}
    self.scan_axis = {k: tuple(permute.index(x) for x in v) for k,v in self.scan_axis.items()}
  ...
  def simplify_ones(self) -> bool:
    ...
    self.local_dims -= sum(all_ones[self.first_scan-self.local_dims:self.first_scan])
    self.scan_axis = {k: tuple(x - sum(all_ones[:x]) for x in v if not all_ones[x]) for k,v in self.scan_axis.items()}
  ...
  def simplify_merge_adjacent(self):
    ...
    for i in range(1, len(shapes[0])):
      ...
      mergeable = all(can_merge) and i != self.first_scan
      changes.append(changes[-1] if mergeable else changes[-1] + 1)
      for j,(s,st) in enumerate(zip(shapes, strides)):
        if mergeable: rets[j][-1] = (rets[j][-1][0] * s[i], st[i])
        else: rets[j].append((s[i], st[i]))
    
    self.scan_axis = {k: tuple(dedup(changes[x] for x in v)) for k,v in self.scan_axis.items()}
    for i,x in enumerate(rets[:len(self.sts)]): self.sts[i] = self.sts[i].reshape(tuple([y[0] for y in x]))

  # ******************** high level optimizers ********************

  def _create_tc_opts(self, scanop:UOp, tc:TensorCore, axis:int, opt_level:int) -> Optional[TensorCoreOptions]:
    has_cast = tc.dtype_in != tc.dtype_out
    if has_cast and not (scanop.src[0].op is Ops.CAST and scanop.src[0].dtype == tc.dtype_out): return None

    mul_op = scanop.src[0].src[0] if has_cast else scanop.src[0]
    ...
    axis_buf0 = [(i,self.full_shape[i],buf1_strides[i]) for i,s in enumerate(buf0_strides[:self.first_scan]) if s == 0]
    axis_buf1 = [(i,self.full_shape[i],buf0_strides[i]) for i,s in enumerate(buf1_strides[:self.first_scan]) if s == 0]
    if not (axis_buf0 and axis_buf1 and ((self.shape_len-self.first_scan) == 1 or (opt_level >= 1))): return None

    axis_choices = list(itertools.product(axis_buf0, axis_buf1, range(self.first_scan, self.shape_len)))
    ...
    self.bufs_for_tensor_core[scanop] = (buf0, buf1)
    if DEBUG >= 3: print("TENSOR CORES", axis_buf0, axis_buf1, tc)
    return TensorCoreOptions(axes=(s0, s1, s2), axes_exist=(True, True), axis_pads=axis_pads)

  def _apply_tc_opt(self, use_tensor_cores:int, axis:int, opt_level:int) -> bool:
    if use_tensor_cores and self.scanop is not None and self.scanop.arg[0] is BinaryOps.ADD:
      for tc in self.opts.tensor_cores:
        ...
        for tc_dim, amt in tc.reduce_axes: self.apply_opt(Opt(OptOps.UNROLL,tc_opts.axes[2]-self.first_scan,amt), append_opt=False)
  ...
  def apply_opt(self, opt:Opt, append_opt:bool=True):
    ...
    if self.scanop is not None and (opt.op in {OptOps.GROUP, OptOps.GROUPTOP} or \
                                      (self.group_for_scans and opt.op not in {OptOps.NOLOCALS, OptOps.PADTO})):
      acc_sz = self.scanop.dtype.itemsize
      upcast_sz = prod([a for a,b in zip(self.full_shape[self.first_upcast:], self.sts[0].shape[self.first_upcast:]) if a == b])
      local_sz = prod(self.full_shape[self.first_scan-self.local_dims:self.first_scan+self.group_for_scans])
    ...
    if opt.op is OptOps.LOCAL:    # cyan
      ...
      self.shift_to(axis, amt, insert_before=self.first_scan)
      self.local_dims += 1
    elif opt.op in {OptOps.GROUP, OptOps.GROUPTOP}:   # green
      check(self.opts.has_local and self.opts.has_shared, "target does not support local or shared mem")
      check(self.first_scan + self.group_for_scans <= axis < self.first_upcast, "must be reduce axis to group")
      check(not self.tensor_core, "can't group with tensor cores")
      check(len(scan_axes:=[i for s in self.scanops for i in s.axis_arg]) == len(set(scan_axes)), "can't group with parallel reduces")
      if all(scanop.arg[2] for scanop in self.scanops):
        self.shift_to(axis, amt, top=(opt.op is OptOps.GROUPTOP), insert_before=self.first_scan + self.group_for_scans)
        self.group_for_scans += 1
      ...
      #if self.full_shape[axis] == amt and axis == self.first_scan: self.local_dims += 1 # first_scan will ++, so offset loss in simplify_ones
      if self.full_shape[axis] == amt and axis < self.first_scan+self.group_for_scans: self.group_for_scans -= 1 # fully unrolling a GROUP
      self.shift_to(axis, amt, insert_before=None)
      self.upcast()
    elif opt.op is OptOps.UPCAST:                     # yellow
      check(axis < self.first_scan, "upcast is for non-reduce")
      ...
      check(self.bufs[0].src[0].dtype.name.startswith('image') and not self.float4_axis(0) and self.group_for_scans != 0 and self.first_scan <= 2 and prod(self.sts[0].shape) > 1, "invalid upcast mid reduce")  # noqa: E501
      ...
      self.shift_to(axis, amt, insert_before=self.first_scan + self.group_for_scans)
      self.group_for_scans += 1
    elif opt.op is OptOps.NOLOCALS:
      check(self.opts.has_local and not self.dont_use_locals, "NOLOCALS is meaningless if target does not support local or already not using locals")
      check(self.local_dims == 0 and self.group_for_scans == 0, "can't have no locals with locals")
      ...
      if (r:=self.scanop) is not None and self.first_scan <= axis: check(r.arg[0] is BinaryOps.ADD and can_pad(r), f"cannot pad {r}")
      padded = False
  ...
  def hand_coded_optimizations(self) -> Kernel:
    ...
    if self.opts.has_local and getenv("MV",1) != 0 and (MV_BLOCKSIZE > 1 or MV_THREADS_PER_ROW > 1 or MV_ROWS_PER_THREAD > 1) and  \
        self.scanop is not None and self.scanop.arg[0] is BinaryOps.ADD and len(self.full_shape) >= 2 and self.opts.has_shared and \
        (mulop:=self.scanop.src[0]).op is BinaryOps.MUL and mulop.src[0].op is Ops.LOAD and mulop.src[1].op is Ops.LOAD:
      ...
      if strides0[self.first_scan] == 1 and not (has_expanded_axis(st0.shape, strides0) and has_expanded_axis(st1.shape, strides1)):
        for global_idx in range(self.global_dims):
          if self.full_shape[self.first_scan]%MV_THREADS_PER_ROW == 0 and self.full_shape[global_idx]%(MV_BLOCKSIZE*MV_ROWS_PER_THREAD) == 0:
            if DEBUG >= 3:
              print(f"MATVEC: {self.full_shape=} {self.first_scan=} {strides0=} {MV_BLOCKSIZE=} {MV_THREADS_PER_ROW=} {MV_ROWS_PER_THREAD=}")
    ...
    if self.opts.has_local and self.opts.has_shared and all_int(self.sts[0].shape[:self.first_scan]):
      # are we grouping? (requires local shape support)
      if not self.float4_axis(0) and self.first_scan <= 2 and self.first_scan + 1 <= self.shape_len and prod(self.sts[0].shape[:self.first_scan]) <= 2048:  # noqa: E501
        # TODO: use 1024 if it's allowed in a smarter way
        for sz in ([256, 16] if prod(self.sts[0].shape[:self.first_scan]) <= 32 else [16]):
          if all(st.shape[self.first_scan] % sz == 0 or st.shape[self.first_scan] == 1 for st in self.sts):
      ...
      if self.bufs[0].src[0].dtype.name.startswith('image') and not self.float4_axis(0) and self.group_for_scans and self.first_scan <= 2 and prod(self.sts[0].shape) > 1:  # noqa: E501
        axes = self.sts[0].unit_stride_axes()
    ...
    for buf_index,buf in enumerate(self.bufs):
      ...
      if buf.src[0].dtype.__class__ is ImageDType:
        ...
        if len(unit_stride_axes_mul_4) and all(x < self.first_upcast for x in unit_stride_axes_mul_4) and unit_stride_axes_mul_4[0] not in self.upcast_in_mid_reduce_axes:  # noqa: E501
          if unit_stride_axes_mul_4[0] < self.first_scan:
            self.apply_opt(Opt(OptOps.UPCAST, unit_stride_axes_mul_4[0], 4))
          else:
            self.apply_opt(Opt(OptOps.UNROLL, unit_stride_axes_mul_4[0]-self.first_scan, 4))

    # no more opt if we are grouping
    if self.group_for_scans: return self
    ...
    for axis in range(self.first_scan):
    ...
    upcasted_axis = set()
    while resolve(prod(self.sts[0].shape[:self.first_scan]) >= 1024):
      xb_choices = []
      for axis, upcast_amount in itertools.product(range(self.first_scan), [3,4]):   # consider all the non reduce axes, and a 3 or 4 reduce
    ...
    if self.first_scan < self.first_upcast and (prod(self.full_shape[self.first_upcast:]) <= 4 or not any(r for _,_,r in self.upcasted_axis(self.full_buf_index))) and (self.upcasted == 0 or prod(self.full_shape[-self.upcasted:]) < 64):  # noqa: E501
      if isinstance(s:=self.full_unupcasted_shape[-1], int) and s <= 32:  # NOTE: cannot loop unroll symbolic axis
        self.apply_opt(Opt(OptOps.UNROLL, len(self.full_unupcasted_shape)-1-self.first_scan, 0))
        # if it's small, upcast a second reduce dimension too
        if self.first_scan < self.first_upcast and s <= 3 and isinstance(s2:=self.full_unupcasted_shape[-1], int) and s2 <= 3:
          self.apply_opt(Opt(OptOps.UNROLL, len(self.full_unupcasted_shape)-1-self.first_scan, 0))
      else:
        for splits in [4]:
          if self.full_unupcasted_shape[-1]%splits == 0:
            self.apply_opt(Opt(OptOps.UNROLL, len(self.full_unupcasted_shape)-1-self.first_scan, splits))
            break
    ...
    if self.opts.has_local:
      if getenv("NOLOCALS") and self.local_dims == 0 and not self.group_for_scans:
        self.apply_opt(Opt(OptOps.NOLOCALS))
      else:
        # prioritize making expand axes local
        local_axis_ranking = [(any(self.sts[buf_index].views[-1].strides[axis] == 0 for buf_index in range(len(self.sts))), axis) for axis in range(len(self.full_shape[:self.first_scan]))]  # noqa: E501
  ...
  def name(self) -> str:
    ...
    kernel_type = "r" if self.scanop is not None else ("C" if all(x.op in GroupOp.Buffer for x in self.ast.parents) else "E")
  ...
  def get_optimized_ast(self) -> UOp:
    # set the shapetrackers to the optimized ones, fixup scanop
    ...
    def fixup_ast(op:UOp, apply_to_st=None) -> UOp:
      ...
      if op.op is Ops.SCAN_AXIS:
        scan_idx = self.scanops.index(op)
        alu_op: Ops = op.arg[0]
        only_reduce: bool = op.arg[2]
        axis = tuple(a for a in self.scan_axis[self.scanops[scan_idx]] if a >= self.first_scan + self.group_for_scans)
        if op in self.bufs_for_tensor_core and (tc := self.tensor_core):
          ssrc = op.src[0]
          if ssrc.op is Ops.CAST: ssrc = ssrc.src[0]
          assert ssrc.op is Ops.MUL
          ...
          if self.use_tensor_cores >= 2:
            if self.use_tensor_cores == 3:
              ...
              ex_shape = tuple(1 if i < self.global_dims or (i >= self.first_scan and i < self.first_upcast) else s \
                              for i,s in enumerate(self.full_shape))
              srcs = []
              for i,(src,fix_st_fxn) in enumerate(zip(ssrc.src, [fix_st1, fix_st2])):
                st_load = [self.sts[self.bufs.index(op)].real_strides() for op in ssrc.parents if op.op is Ops.LOAD]
              ...
              srcs = [fixup_ast(ssrc.src[0]), fixup_ast(ssrc.src[1])]
            # MUL/SUM instead of WMMA
            ret = UOp(Ops.SCAN_AXIS, tc.dtype_out, (srcs[0].alu(BinaryOps.MUL, srcs[1]).cast(tc.dtype_out),), (alu_op, wmma_arg[-1], only_reduce))
          else:
            ...
            wmma = UOp(Ops.WMMA, dtype=tc.dtype_out.vec(wmma_sz[2]), src=(
              UOp(Ops.CONTRACT, dtype=ssrc.src[0].dtype.vec(wmma_sz[0]), src=(fixup_ast(ssrc.src[0], fix_st1),), arg=wmma_upcast_axes[0]),
              UOp(Ops.CONTRACT, dtype=ssrc.src[1].dtype.vec(wmma_sz[1]), src=(fixup_ast(ssrc.src[1], fix_st2),), arg=wmma_upcast_axes[1]),
        ...
        if self.group_for_scans:
          start = UOp(Ops.SCAN_AXIS, op.dtype, (fixup_ast(op.src[0], apply_to_st),), arg=(alu_op, axis, only_reduce))
          second_axis = tuple(a for a in self.scan_axis[self.scanops[scan_idx]] if a < self.first_scan + self.group_for_scans)
          if len(second_axis) == 0: return UOp(Ops.SCAN_AXIS, op.dtype, (fixup_ast(op.src[0], apply_to_st),), arg=(alu_op, axis, only_reduce))
          # NOTE: if there's a grouped reduce, but no reduce axes for this reduce, we can skip it
          if len(second_axis) == 0: return start
          local_shape = (1,) * self.global_dims + self.full_shape[self.global_dims:self.first_scan] + \
            tuple([self.full_shape[i] if i in second_axis else 1 for i in range(self.first_scan, self.shape_len)])
          st_uop = ShapeTracker.from_shape(local_shape).to_uop()
          local_buffer = UOp(Ops.DEFINE_LOCAL, op.dtype.ptr(local=True), (), (f"temp{self.scanops.index(op)+1}", st_uop.arg.real_size()))
          local_load = UOp(Ops.LOAD, op.dtype, (local_buffer, st_uop, UOp.store(local_buffer, st_uop, start)))
          grouped_scan = UOp(Ops.SCAN_AXIS, op.dtype, (local_load,), arg=(alu_op, second_axis, only_reduce))
          if op is self.scanops[-1]: return grouped_scan
          st_uop = ShapeTracker.from_shape(tuple([1 if i in second_axis else a for i,a in enumerate(local_shape)])).to_uop()
          return UOp(Ops.LOAD, op.dtype, (local_buffer, st_uop, UOp.store(local_buffer, st_uop, grouped_scan)))
        arg = (alu_op, axis, only_reduce)
      elif op.op is Ops.SINK:
        src = tuple(fixup_ast(x, apply_to_st) for x in op.src)
        arg = KernelInfo(self.local_dims, self.upcasted, self.dont_use_locals, self.first_scan, self.group_for_scans)
      src = src if src is not None else tuple(fixup_ast(x, apply_to_st) for x in op.src)
      return op.replace(src=src, arg=arg)
...
def _assert_valid_uop(uop:UOp, st:ShapeTracker, sts:Dict[UOp, ShapeTracker]) -> None:
  ...
  if uop.op is Ops.LOAD and uop.src[0].op is Ops.DEFINE_LOCAL:
    ...
    return
  # if uop.op is Ops.LOAD and uop.src[0].op is Ops.DEFINE_LOCAL:
  #   _assert_valid_uop(uop.src[2].src[2], uop.st_arg, sts)
  #   sts[uop] = uop.src[1].arg
  #   return
  for x in uop.src: _assert_valid_uop(x, st, sts)
  # only reduceuop is allowed to change shape, limited to turning n to 1
  if uop.op is Ops.WMMA or (uop.op is Ops.SCAN_AXIS and uop.arg[2]) : st = ShapeTracker.from_shape(sts[uop.src[0]].reduce(uop.axis_arg))
